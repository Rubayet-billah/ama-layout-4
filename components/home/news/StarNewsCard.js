...
const StarNewsCard = ({ news }) => {
  return (
    <div className="flex flex-col h-full mx-auto bg-white border rounded shadow-lg">
      <div className="relative">
        <MRFImage
          src={news?.category?.banner || defaultThumb}
          classNames="w-full h-40 lg:h-96 rounded-t-lg"
      ...
      <div className="flex flex-col flex-1 p-5 text-sm">
        <div className="flex-grow">
          <p>{news?.category}</p>
          <h1 className="mt-2 text-base font-semibold lg:text-xl">
            <Link
